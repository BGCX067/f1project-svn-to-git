...
class DB_SQL
{
	...
	function Connect($data="")
	{
		...
		$this->conn = mysqli_connect($this->host,$this->user,$this->pass) or die(mysqli_errno().": ".mysqli_error());

		$mysqli_version = $this->GetVersion();
	...
	function SelectDb($data="")
	{
		...
		$this->data = $database;
		$starttime = $this->TimeUsed();
		@mysqli_select_db($this->conn,$this->data) or die(mysqli_errno().": ".mysqli_error());
		$endtime = $this->TimeUsed();
		$this->connTimes += round($endtime - $starttime,5);#[连接数据库的时间]
	}
	...
	function Set($name,$value)
	{
		if($name == "rsType")
		{
			$value = $value == "NUM" ? MYSQLI_NUM : MYSQLI_ASSOC;
		}
		$this->$name = $value;
	}
	...
	function GetAll($sql="",$primary="")
	{
		...
		$starttime = $this->TimeUsed();
		while($rows = mysqli_fetch_array($result,$this->rsType))
		{
	...
	function GetOne($sql="")
	{
		...
		$rows = mysqli_fetch_array($result,$this->rsType);
		$endtime = $this->TimeUsed();
		$this->queryTimes += round($endtime - $starttime,5);#[查询时间]
		return $rows;
	}
	...
	function Count($sql="")
	{
		if($sql)
		{
			$this->rsType = MYSQLI_NUM;
			$this->Query($sql);
			$rs = $this->GetOne();
			$this->rsType = MYSQLI_ASSOC;
			return $rs[0];
		}
		else
		{
	...
	function EscapeString($char)
	{
		...
		return @mysqli_escape_string($char);
	}
