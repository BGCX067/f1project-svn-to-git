...
<!-- run:$playerlist = QGMOD_PLAYER() -->
<!-- if($playerlist && count($playerlist)>0 && is_array($playerlist)) -->
...
var widths=516;
var heights=126;
var counts={run:echo count($playerlist)};//获取翻页图片数量
<!-- $playerlist AS $key=>$value -->
img{:$key}=new Image();
img{:$key}.src="{:$value[img]}";
url{:$key}=new Image ();
url{:$key}.src="{:$value[url]}";
<!-- end -->
var nn=1;
var key=0;
function change_img()
